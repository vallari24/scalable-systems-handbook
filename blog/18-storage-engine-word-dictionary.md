...
**Who actually drives the swap?** Strategy 3 describes the *files*, but not who moves them — and since we <span style="color:#9aa6b2"><strong>manage our own infra</strong></span>, something has to. That something is the <span style="color:#ff8bd2"><strong>throwaway merge worker</strong></span> from Section 6: it already did the merge and already knows it placed `002/` in S3. So we give it two more jobs — flip `meta.json`, then signal the API fleet — and then it disposes of itself.

<img src="../assets/word-dictionary/publish-rollout.svg" alt="Who actually triggers the swap: the rollout sequence, drawn as three lanes — a throwaway merge worker (you manage your own infra), S3 (the static store holding meta.json plus versions), and the API server fleet (stateless, index held in RAM). Step 1, the worker uploads 002/index.dat and 002/data.dat to S3. Step 2, the worker flips meta.json from 001 to 002 in one atomic write. Step 3, the worker triggers a reboot of the fleet — signalling the new version is live — and is then thrown away. Each API server now boots: step 4, on boot it reads meta.json, the static file, from S3; step 5, S3 returns the current version, 002 slash, the new path; step 6, the server loads 002/index.dat and rebuilds its in-memory cache; step 7, it serves requests from 002. The bottom note: nothing is overwritten and no server is patched live — old servers drain on 001 while rebooted ones come up on 002 by re-reading meta.json, so the index in RAM is always rebuilt from the version the pointer names." width="1000">

Flipping `meta.json` is the <span style="color:#93c5fd"><strong>atomic operation</strong></span> — one pointer write, and from that instant any server that *reads* `meta.json` sees `002/`. A server already running still holds `001/`'s index in RAM, so it picks up the new version by <span style="color:#9aa6b2"><strong>rebooting</strong></span>: on boot it reads the static <span style="color:#93c5fd"><strong>`meta.json`</strong></span>, learns the current path is `002/`, loads that index, and <span style="color:#8aff8a"><strong>rebuilds its in-memory cache</strong></span> before taking traffic. A fresh boot can't mix old offsets with new data, because it never reloads anything mid-flight — it just starts on whatever version the pointer names. <span style="color:#9aa6b2"><strong>Old servers</strong></span> drain `001/` until they cycle out; new ones come up on `002/`.

> **Memory hook:** *flip `meta.json` atomically to publish the new version, then reboot the fleet so each server rebuilds its in-RAM index fresh from the version the pointer names.*
